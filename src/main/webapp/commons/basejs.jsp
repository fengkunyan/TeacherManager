<%--标签 --%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ include file="/commons/global.jsp" %>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta http-equiv="X-UA-Compatible" content="edge" />
<link rel="shortcut icon" href="${staticPath }/favicon.ico" />
<%-- [EasyUI] --%>
<link rel="stylesheet" type="text/css" href="${staticPath }/js/jquery-easyui/themes/gray/easyui.css">
<link rel="stylesheet" type="text/css" href="${staticPath }/js/jquery-easyui/themes/icon.css">
<link rel="stylesheet" type="text/css" href="${staticPath }/js/jquery-easyui/themes/color.css">
<link rel="stylesheet" type="text/css" href="${staticPath }/css/train.css">
<link rel="stylesheet" type="text/css" href="${staticPath }/layui/css/layui.css">

<script type="text/javascript" src="${staticPath }/js/jquery-easyui/jquery.min.js"></script>
<script type="text/javascript" src="${staticPath }/js/jquery-easyui/jquery.easyui.min.js"></script>
<script type="text/javascript" src="${staticPath }/js/jquery-easyui/locale/easyui-lang-zh_CN.js"></script>
<script type="text/javascript" src="${staticPath }/layui/layui.all.js"></script>


<script type="text/javascript" src="${staticPath }/js/train/train.js"></script>
